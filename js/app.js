  ...
  CHANGELOG: [
    {
      ...
      date: '2026-08-21',
      changes: [
        '重大核心设定更新：将94条《龙族》世界模拟器完整核心规则整合到AI系统提示词中',
        '新增20大核心引擎角色：世界状态模拟器、原著时间线管理器、正典版本控制器、NPC人格与行为引擎、混血种社会模拟器、势力政治模拟器、调查与情报系统、任务设计与任务裁决器、冲突与战斗裁决器、长期因果关系管理器、世界事件调度器、人际关系网络管理器、秘密与信息传播管理器、伏笔与悬念管理器、原著事件重模拟器、长期记忆压缩器、存档与续玩管理器、OOC一致性审查器、世界设定一致性审查器、长篇剧情节奏控制器',
        '新增规则优先级系统（P0-P10）：玩家元指令>已锁定世界事实>当前真实世界状态>人物真实状态>已发生历史事件>正典版本>世界底层规则>NPC人格>时间线>原著趋势>戏剧效果',
        '新增正典版本锁系统：CANON-R修订主线（默认）、CANON-O旧版连续线、CANON-H混合模式、CANON-C自定义世界线',
        '新增正典标签系统：R-CANON、O-CANON、REBOOT、CANON-INFER、AMBIGUOUS、CUSTOM',
        '新增原著事件锚点系统（Canon Event Graph）：ARC-00历史阴影、ARC-01卡塞尔之门、ARC-02悼亡者之瞳、ARC-03黑月之潮、ARC-04A奥丁之渊旧版、ARC-04R世界重启、ARC-05旧V资料库',
        '新增历史惯性机制：大型事件由多个独立原因共同造成，改变一个因素通常不足以让事件消失',
        '新增世界状态五层结构：L0世界规则层、L1世界事实层、L2隐藏真相层、L3动态世界层、L4场景状态层',
        '新增世界事实表（World Fact Ledger）：所有已确定事实永久记录，普通叙事不得覆盖',
        '新增因果链系统（Causal Node）：重要行为产生直接影响、二级影响、潜在长期影响，长期影响随时间逐渐出现',
        '新增蝴蝶效应等级系统：B0无偏差→B1微小→B2人物级→B3支线级→B4主线节点→B5多事件链→B6世界线重构',
        '新增世界时钟系统：始终维护唯一时间，所有行为产生合理时间成本，世界不会等待玩家',
        '新增Parallel World Tick：每次重要场景结束执行后台世界推进，检查12项世界状态',
        '新增长时间跳跃系统：超过7天执行Long-Term Tick，超过三个月额外计算重要人物长期行动',
        '新增NPC独立人格系统：24项NPC属性完整维护，包括公开身份、真实身份、公开目标、真实目标、Character Core等',
        '新增Character Core人格锁：重要原著人物必须保持人格连续性，人格变化必须来自事件、压力、失败等',
        '新增16位核心人物行为锚点：路明非、楚子航、恺撒、诺诺、芬格尔、昂热、夏弥、零、源稚生、源稚女、绘梨衣、路鸣泽等',
        '新增NPC信息隔离系统：KNOWN、BELIEVED、SUSPECTED、RUMOR、UNKNOWN、FALSE六级信息状态',
        '新增谎言与欺骗系统：Information Provenance记录每条信息来源、可信度、证据、传播链',
        '新增Secret Ledger秘密账本：所有核心秘密进入账本，6种状态（HIDDEN/LEAKING/SUSPECTED/PARTIAL/EXPOSED/DESTROYED）',
        '新增12维人际关系系统：熟悉、信任、尊重、好感、戒备、利益一致、依赖、责任、秘密共享、人情、冲突',
        '新增NPC↔NPC关系网络：必须维护NPC之间的关系，玩家改变一个NPC可能间接改变其他人物关系',
        '新增矛盾网络系统（Conflict Web）：一个人物可以同时处在3-5组矛盾之中，不是二元阵营',
        '新增势力系统：12项势力属性完整维护，至少追踪13个重要组织',
        '新增组织不是一个人原则：不同派系可以同时拥有完全不同态度',
        '新增暴露度系统（Exposure）：EX0无人注意→EX5世界级异常目标',
        '新增组织怀疑度系统：不同组织分别维护SUSPICION，不是统一数值',
        '新增血统系统：12项血统属性完整维护，血统不是等级，学院评级只是社会评估',
        '新增血统代价系统：越接近龙类力量越强，但稳定度、精神压力、失控风险同步提高',
        '新增言灵系统：18项言灵属性完整记录，原创言灵必须符合龙族世界风格',
        '新增能力未知原则：机制存在争议的能力标记Mechanism Unknown，通过剧情继续探索',
        '新增战斗裁决系统：基于15项因素判定战斗结果，随机只影响边界情况',
        '新增战斗状态机：8阶段战斗流程，只有真正重要的决策点才暂停询问玩家',
        '新增尼伯龙根系统：每个尼伯龙根必须拥有9项属性，不是随机副本',
        '新增调查系统（Clue Graph）：13种线索类型，每条线索11项属性完整记录',
        '新增真相不能靠单一线索获得原则：重大秘密需要多个相互独立的证据',
        '新增任务结构系统：18项任务属性完整维护，任务不是打怪领奖',
        '新增任务阶段系统：12阶段任务流程，不是每个任务都完整经历',
        '新增悲剧拆解机制（Tragedy Causal Chain）：悲剧由多个因果链造成，阻止悲剧必须真正破坏因果链',
        '新增救下一个人以后世界继续原则：救人成功本身只是新的因果起点',
        '新增伏笔系统（Open Thread Registry）：6种伏笔状态，伏笔不能全部立即回收也不能永久遗忘',
        '新增伏笔质量原则：优秀伏笔应满足第一次自然、第二次异常、第三次形成模式',
        '新增隐藏剧情系统：依赖11种条件触发，未满足条件不展示',
        '新增玩家成长系统：14项成长维度，不是只有战斗力',
        '新成长需要时间原则：记录训练、实战、课程等，禁止连续几天内无原因大幅成长',
        '新增资源系统：15种玩家资源，一个秘密可能比现金重要',
        '新增玩家身份系统：14种可选身份，也允许穿越者模式',
        '新增原著知识≠世界证据原则：穿越者记得原著但NPC不一定相信',
        '新增原创角色限制：原创人物不得无解释强于核心人物、掌握全部真相、拥有万能言灵等',
        '新增原创剧情原则：利用龙族世界已存在的矛盾，不是随机生成玄幻副本',
        '新增决策等级系统：D0日常→D5改变世界线，普通回合最多一个核心决策',
        '新增不要把互动小说做成选择题原则：玩家始终可以自由输入，选项只是建议',
        '新增选项必须具有代价原则：每个选项说明具体行动和代价后果',
        '新增世界不围绕玩家运行原则：NPC会主动执行任务、调查、学习、竞争等',
        '新增NPC主动事件系统：满足条件时NPC可以主动发消息、邀请、请求帮助等',
        '新增防主角光环原则：玩家开局不是世界中心，名声必须通过行动逐渐建立',
        '新增防攻略角色化原则：任何原著角色首先是具有独立人生的人',
        '新增情绪状态系统：情绪影响行为倾向但不能控制全部行为',
        '新增剧情节奏系统：13种内容类型切换，禁止连续大量回合只有聊天或战斗',
        '新增防拖沓原则：连续3轮没有产生新变化必须推进至少一项',
        '新增Recent Event Cache：记录最近约10轮主要事件，避免重复偶遇、重复任务、重复袭击',
        '新增龙族气质原则：体现青春、孤独、热血、荒诞、遗憾、成长等，但不要模仿原作者句式',
        '新增悲剧原则：悲剧优先来自错过、误解、立场等，不要依赖随机死亡',
        '新增对话原则：所有人物根据身份、教育、年龄等说话，禁止所有NPC都像谜语人',
        '新增叙事视角原则：默认第二人称有限视角，偶尔允许镜头切换但不得泄露核心秘密',
        '新增正常输出格式：时间地点标题、正文剧情、状态变化提示、决策选项、简要状态',
        '新增无决策回合原则：没有真正重要的决定不要强行生成选项',
        '新增PLAYER STATE系统：24项玩家属性完整维护',
        '新增世界事件队列（World Event Queue）：6种事件等级，玩家不在场事件仍然可能发生',
        '新增场景生命周期系统：10项场景属性，场景目标完成后结束Scene',
        '新增Scene Commit：场景结束后更新12项世界状态，然后生成下一场景',
        '新增单轮执行协议：12步标准流程，从判断玩家意图到只输出玩家能够获得的信息',
        '新增禁止泄露后台原则：普通游玩禁止显示未来事件、NPC隐藏目标、隐藏数值等',
        '新增长期记忆系统：Memory-A核心事实、Memory-B活跃剧情、Memory-C场景、Memory-D Canon State',
        '新增Rolling Summary：每约10轮内部生成摘要，只保存高价值信息',
        '新增Consistency Audit：每15-20轮执行一致性检查，检查20项世界状态',
        '新增自动存档系统：9种情况建立AUTO SAVE',
        '新增玩家存档系统：玩家输入"存档"建立SAVE-XXX，保存15项世界状态',
        '新增龙族续玩码系统：完整续玩码格式，信息优先、文学描写最少、可以跨聊天恢复',
        '新增读取续玩码系统：检测到续玩码立即读取状态，不要重新创建角色',
        '新增超长篇章压缩：游戏超过50轮按ARC保存，之后主要保留ARC SUMMARY',
        '新增查询指令系统：15种查询指令，仅展示玩家角色能够合理知道的信息',
        '新增节奏指令系统：10种节奏指令，用于调整叙事节奏，不得改变世界事实',
        '新增玩家纠错协议：玩家说"你记错了"立即暂停推进，检查存档后按照玩家修正',
        '新增开局时间选择：7种时间选项，根据Canon Profile自动解释世界状态',
        '新增开局身份：6种身份选项，第一次游戏只询问少量信息',
        '新增角色创建流程：3-5轮完成创建，玩家一次性给完直接创建',
        '新增血统开局平衡：默认不允许无代价S级，极端特殊身份必须同时创建代价和限制',
        '新增玩家档案确认：创建完成生成PLAYER PROFILE，玩家确认后世界时间开始运行',
        '新增开局事件原则：不要让玩家创建完成马上遇见核心角色或触发龙王事件',
        '新增游戏失败原则：允许任务失败、关系破裂等，失败不是游戏结束',
        '新增极端后果原则：重大后果必须来自明确风险、合理因果、此前线索',
        '新增反套路黑名单：22种禁止行为，包括第一次见面就说玩家很特殊、所有核心角色自动喜欢玩家等',
        '新增最终运行原则：玩家决定行动，世界决定结果，原著只是这个世界在玩家出现之前原本可能抵达的未来',
        '所有现有功能分支保持不变：五维数值系统、背包高灵敏度判定、关系/势力/世界红点弹窗、左滑隐藏功能、PANEL_UPDATE机制、11大板块AI自动托管等全部保留'
      ]
    },
    {
      version: 'v4.3.11',
